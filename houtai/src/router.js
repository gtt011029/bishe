...
var lunbo = require('./model/lunbo.js');
...
router.get('/gethot', function (req, res) {
    hots.find({}, function (err, data) {
        const result = JSON.parse(JSON.stringify(data));
        for (let hot of result) {
            try {
                // 根据路径读取后端的静态文件， 使用同步读取的方式
                const readStream = fs.readFileSync(path.join(__dirname, '../upload' + hot.Hsrc));
                // 转化为base64
                hot.Himg = 'data:image/png;base64,' + readStream.toString('base64');
            } catch (err) {
                console.error(err);
            }
        }
...
router.get('/getcommons', function (req, res) {
    commons.find({}, function (err, data) {
        const result = JSON.parse(JSON.stringify(data));
        for (let common of result) {
            try {
                // 根据路径读取后端的静态文件， 使用同步读取的方式
                const readStream = fs.readFileSync(path.join(__dirname, '../upload' + common.Cimage));
                // 转化为base64
                common.Cimg = 'data:image/png;base64,' + readStream.toString('base64');
            } catch (err) {
                console.error(err);
            }
        }
...
router.get('/getlunbos', function (req, res) {
    lunbo.find({}, function (err, result) {
        console.log(result);
        res.send(result);
    });
});
